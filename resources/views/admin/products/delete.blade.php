...
<div id="layoutSidenav_content" class="p-4 bg-gray-50 min-h-screen">
    ...
    <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between mb-4 gap-3">
        <h1 class="text-2xl font-bold text-gray-800 flex items-center">
            <i class="fas fa-box-open text-red-600 mr-2"></i> Gestión de Productos Eliminados
        </h1>
                ...
                <table class="table table-hover align-middle text-sm">
                    <thead class="bg-gray-100 text-gray-700 text-xs uppercase">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-right">Precio (Bs)</th>
                            <th class="px-3 py-2">Usuario</th>
                            <th class="px-3 py-2">Estado</th>
                            <th class="px-3 py-2">Fecha Publicación</th>
                    ...
                    <tbody>
                        ...
                        @forelse ($productos as $producto)
                            <tr class="hover:bg-green-50 transition">
                                ...
                                <td class="px-3 py-2">{{ $producto->title }}</td>
                                <td class="px-3 py-2">{{ ucfirst(str_replace('_',' ',$producto->type)) }}</td>
                                <td class="px-3 py-2 text-right">{{ number_format($producto->price, 2) }}</td>
                                <td class="px-3 py-2">{{ $producto->user->name ?? 'No asignado' }}</td>
                                <td class="px-3 py-2">
                                    <span class="px-2 py-1 text-xs rounded-full bg-red-200 text-red-800 font-medium">Inactivo</span>
                                </td>
                                <td class="px-3 py-2">{{ \Carbon\Carbon::parse($producto->created_at)->format('d/m/Y') }}</td>
                                <td class="px-3 py-2 text-center">
                                    <form action="{{ route('products.activate', $producto->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" 
